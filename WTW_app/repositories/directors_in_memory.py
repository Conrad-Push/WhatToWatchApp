...
class DirectorsInMemory(IDirectorsRepository):
    ...
    def get_director_details(
        ...
        *, 
        ...
        if _director:
            logger.info(f"A director's details for the director id: '{director_id}' have been found.")
            return _director
        else:
            logger.info(f"No director's details for the director id: '{director_id}' have been found.")
            return None
    ...
    def add_director(
        ...
        *, 
        ...
        _director_exists: tp.List[DirectorModel] = [
            _director
            for _director in self.storage.values()
            if _director.name == name
        ]
        
        if _director_exists:
            logger.warning(
                f"Director with name: '{name}' already exists!"
            )
            return None
        
        _director_id: int = self.directors_sequence + 1
        _director: DirectorModel = DirectorModel(
