...
from WTW_app.cassandra.dependencies import get_database_repository

cassandra_database_router = APIRouter(
    prefix="/cassandra/database",
    tags=["Database - Cassandra"],
)


@cassandra_database_router.get("/status", response_model=DatabaseInfoResponse)
...
@cassandra_database_router.get("/tables/restart", response_model=DatabaseInfoResponse)
...
@cassandra_database_router.post("/data/generate", response_model=DataGenerationResponse)
def generate_data(
...
@cassandra_database_router.post("/data/scrap", response_model=DataScrappingResponse)
def scrap_data(
