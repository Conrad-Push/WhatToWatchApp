...
def get_director_details(director_id:int) -> tp.Optional[DirectorModel]:
    ...
    _director_details: tp.Optional[DirectorModel] = directors_repository.get_director_details(director_id=director_id)
...
@directors_router.post("/", response_model=DirectorModel, status_code=status.HTTP_201_CREATED)
...
def modify_director_details(director_id: int, director_payload: PatchDirectorModel) -> tp.Optional[DirectorModel]:
    directors_repository: IDirectorsRepository = directors_in_memory
...
@directors_router.delete("/{director_id:int}", response_model=DirectorModel, responses={404: {"description": "Director for given director id not found."}})
async def delete_director(director_id: int) -> tp.Optional[DirectorModel]:
    directors_repository: IDirectorsRepository = directors_in_memory
    
    _director: tp.Optional[DirectorModel] = directors_repository.remove_director(director_id==director_id)
